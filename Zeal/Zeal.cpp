...
ZealService::ZealService() {
  ...
  gamestr_hook = MakeCheckedShared(GameStr);
  ...
  render_preset = MakeCheckedShared(RenderPreset);
...
void ZealService::configuration_check() {
  ...
  std::string xml_files[] = {
      "EQUI_GuildManagementWnd.xml", "EQUI_OptionsWindow.xml",   "EQUI_Tab_Cam.xml", "EQUI_Tab_Colors.xml",
      "EQUI_Tab_FloatingDamage.xml", "EQUI_Tab_General.xml",     "EQUI_Tab_Map.xml", "EQUI_Tab_Nameplate.xml",
      "EQUI_Tab_TargetRing.xml",     "EQUI_ZealInputDialog.xml", "EQUI_ZealMap.xml", "EQUI_ZealOptions.xml"};
  ...
  if (not filepathExists) {
    std::wstring missing =
        L"The directory 'zeal' is missing from the uifiles directory.\nZeal will not function properly!";
    ...
    if (std::filesystem::exists(deprecated_filepath)) {
      std::wstring deprecated_warning =
          L"The deprecated Zeal 'EQUI.xml' file has been detected at the following location:\n" +
...
void ZealService::basic_binds() {
  ...
  binds_hook->replace_cmd(0xC8, [this](int state) {
    ...
    if (item_displays->close_latest_window()) return true;

    return false;
  });  // handle escape
}

ZealService *ZealService::get_instance() { return ptr_service; }
